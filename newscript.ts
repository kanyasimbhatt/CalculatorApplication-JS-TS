...
class Calculator {
  ...
  constructor() {
    ...
    document.querySelectorAll("#M-plus-minus-button").forEach((element) => {
      element.addEventListener("click", (event) => {
        this.handleMplusAndMinus(event.target as EventTarget);
      });
    });
  ...
  handleMplusAndMinus(ref: EventTarget) {
    ...
    document.querySelectorAll(".history-data").forEach((element) => {
      element.addEventListener("click", (event: Event) => {
        if ("id" in event.target!)
          this.handleClickOnHistoryData(event.target.id as string);
      });
    });
  ...
  handleClickOnHistoryData(value: string) {
    handleClosingHistorySection();
    this.regex = /(\d+)$/;
    if (this.regex.test(this.calculatorInput.value)) {
  ...
  handleKeyboardInput(event: Event) {
    ...
    if (validKeyboardCharacters.indexOf(keyValue) !== -1) {
