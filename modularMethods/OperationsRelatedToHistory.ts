...
function handleClosingHistorySection() {
  (
    document.getElementsByClassName("text-box")[0] as HTMLElement
  ).style.display = "flex";
  (
    document.getElementsByClassName("advanced-operations")[0] as HTMLElement
  ).style.display = "block";
  (
    document.getElementsByClassName("calculator-buttons")[0] as HTMLElement
  ).style.display = "grid";
...
export function handleHistory(this: Calculator) {
  (htmlElement.textBoxInput as HTMLInputElement).style.display = "none";
  (htmlElement.advanceOperationsSection as HTMLDivElement).style.display =
    "none";
...
export function handleClickOnHistoryData(this: Calculator, value: string) {
  handleClosingHistorySection();
  this.regex = /(\d+)$/;
...
function showHistoryContent(this: Calculator) {
  document.getElementsByClassName(
    "history-section"
  )[0].innerHTML = `<div class = "history-title-clear-button">
  ...
  document.querySelectorAll(".history-data").forEach((element) => {
    element.addEventListener("click", () => {
      if ("id" in element)
        handleClickOnHistoryData.call(this, element.id as string);
    });
  });
...
function handleClearHistory(this: Calculator) {
