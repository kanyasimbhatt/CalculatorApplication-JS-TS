...
export function replaceAll(newStr, secondOperationToggle) {
    ...
    newStr = newStr.replace(/sin\((.+)\)/g, `Math.sin(${conversionBetweenDegRad}($1)).toFixed(2)`);
    newStr = newStr.replace(/cos\((.+)\)/g, `Math.cos(${conversionBetweenDegRad}($1)).toFixed(2)`);
    newStr = newStr.replace(/tan\((.+)\)/g, `Math.sin(${conversionBetweenDegRad}($1)).toFixed(2)/Math.cos(${conversionBetweenDegRad}($1)).toFixed(2)`);
    newStr = newStr.replace(/cosec\((.+)\)/g, `(1/Math.sin(${conversionBetweenDegRad}($1))).toFixed(2)`);
    newStr = newStr.replace(/sec\((.+)\)/g, `(1/Math.cos(${conversionBetweenDegRad}($1))).toFixed(2)`);
    newStr = newStr.replace(/cot\((.+)\)/g, `Math.cos(${conversionBetweenDegRad}($1)).toFixed(2)/Math.sin(${conversionBetweenDegRad}($1)).toFixed(2)`);
